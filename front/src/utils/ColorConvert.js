...
export function getFontColor(_OldColorValue) {
    if (/[a-zA-Z]/.test(_OldColorValue[0])) {
        OldColorValue = getHexColor(_OldColorValue)
    }
    if (!OldColorValue) {
        return "#000000"
    }
    var OldColorValue = OldColorValue.replace(/#/g, "");
    let r = parseInt(OldColorValue.substring(0, 2), 16);
    let g = parseInt(OldColorValue.substring(2, 4), 16);
    let b = parseInt(OldColorValue.substring(4, 6), 16);
    return ((0.213 * r + 0.715 * g + 0.072 * b) > (255 / 2)) ? '#000' : '#FFF';
}
export function colorReverse(_OldColorValue) {
    try {
        if (/[a-zA-Z]/.test(_OldColorValue[0])) {
            OldColorValue = getHexColor(_OldColorValue)
        }
        ...
        if (["#696969"].indexOf(OldColorValue) >= 0) {
            return "#ffffff"
        }
